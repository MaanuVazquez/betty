  ...
  async execute(interaction, client) {
    try {
      ...
      if (
        ...
        const noPermissionTitle = await LanguageManager.getTranslation(
          interaction.guild.id,
          "commands.language.errortitle",
        );
        const noPermissionDesc = await LanguageManager.getTranslation(
          interaction.guild.id,
          "commands.language.permission_required",
        );
      ...
      for (const file of languageFiles) {
        const langData = JSON.parse(
          fs.readFileSync(path.join(languagesPath, file), "utf8"),
        );
        languages.push({
      ...
      const currentLangFile = JSON.parse(
        fs.readFileSync(
          path.join(languagesPath, `${currentLang}.json`),
          "utf8",
        ),
      );
      ...
      let errorDes = await LanguageManager.getTranslation(
        interaction.guild.id,
        "commands.language.error2",
      );
      let errorTitle = await LanguageManager.getTranslation(
        interaction.guild.id,
        "commands.language.errortitle",
      );
  ...
  async handleLanguageButton(interaction) {
    try {
      ...
      if (
        ...
        const noPermissionDesc =
          "❌ Bu butonu kullanmak için **Sunucuyu Yönet** yetkisine sahip olmalısın!";
      ...
      const success = await LanguageManager.setServerLanguage(
        guildId,
        selectedLang,
      );
      ...
      const selectedLangData =
        await LanguageManager.getLanguageData(selectedLang);
      ...
      const successTitle = await LanguageManager.getTranslation(
        guildId,
        "commands.language.changed",
      );
      const successDescription = await LanguageManager.getTranslation(
        guildId,
        "commands.language.changed_desc",
        {
